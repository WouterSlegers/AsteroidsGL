...
import com.wjslegers.asteroidsgl.Utils.Point3D;
...
import static com.wjslegers.asteroidsgl.config.MAX_NUMBER_POINTS;

public class Mesh {
    private static final String TAG = "Mesh";
    public static final int SIZE_OF_FLOAT = Float.SIZE / Byte.SIZE;// find the size of the float type, in bytes //32bit/8bit = 4 bytes
    public static final int COORDS_PER_VERTEX = 3; // number of coordinates per vertex in our meshes: X, Y, Z
    public static final int VERTEX_STRIDE = COORDS_PER_VERTEX * SIZE_OF_FLOAT;// number of bytes per vertex
    public static final float POINT[] = {0f, 0f, 0f};

    public static final int X = 0;
    public static final int Y = 1;
    public static final int Z = 2;

    private static final PointF[] _pointPool = new PointF[2*MAX_NUMBER_POINTS];
    ...
    public PointF[] getPointList(final float offsetX, final float offsetY, final float facingAngleDegrees){
        ...
        for (int i = 0; i < _vertexCount * COORDS_PER_VERTEX; i += COORDS_PER_VERTEX) {
            ...
            if(_pointPool[index] == null){
                _pointPool[index] = new PointF();
            }
            _pointPool[index].x = (float) (x * cosTheta - y * sinTheta) + offsetX;
            _pointPool[index].y  = (float) (y * cosTheta + x * sinTheta) + offsetY;
            //final float z = verts[i + Z];
            out[index] = _pointPool[index];
            index++;
        }
        return out;
    }
    ...
    //Mesh.java
    public static float[] generateLinePolygon(int numPoints, final double radius) {
        Utils.require(numPoints > 2, "a polygon requires at least 3 points.");
        numPoints = (int) clamp(numPoints, 3, MAX_NUMBER_POINTS);
    ...
    public void setWidthHeight(final double w, final double h) {
        ...
        Utils.expect(Math.abs(w - _width) < Float.MIN_NORMAL && Math.abs(h - _height) < Float.MIN_NORMAL,
